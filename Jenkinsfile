    ...
    stages {
        ...
        stage('🔬 Trivy FS Scan') {
            steps {
                dir("${env.APP_DIR?.trim() ? env.APP_DIR : '.'}") {
                    sh '''
                        # Cache DB in a persistent workspace dir
